...
#include "score.h"
#include "congrats.h"
bool Pathway(int x, int y, int xmax, int xmin, int ymax, int ymin) {
...
int main() {
	cout << "Use wasd then press enter to move || Enter 'e' to escape during battle || Enter 'i' for inventory/stats || Enter 'm' for map \n";
	int x = 1, y = 1;
	int key = 0;
	bool forest = true;
	bool dinning = true;
...
//	final_congratulations();
	...
	while (true) {
		...
		cout << "Use 'w', 'a', 's', 'd' to movie around || use 'm' for map" << endl;
		cin >> walk;
		if (walk == 'a') x = x - 1;
		else if (walk == 'd')x = x + 1;
		else if (walk == 'w')y = y + 1;
		else if (walk == 's')y = y - 1;

		if (walk == 'm') {
			cout << "T = tavern | F = forrest | C = cave | D(2 = second part) = dinning hall | B = boss" << endl;;
			cout << "Y  ________________________________" << endl;
			cout << "20-|                              |" << endl;
			cout << "19-|                              |" << endl;
			cout << "18-|                              |" << endl;
			cout << "17-|                              |" << endl;
			...
			cout << "1--|T                             |" << endl;
			cout << "   ||||||||||||||||||||||||||||||||" << endl;
			cout << "   X123456789111111111122222222223 " << endl;
			cout << "             012345678901234567890 " << endl;

		}
		//cave location and puzzle1
		if (x == 9 and y == 4 and forest == true) {
...
//D2 walls
		if (Pathway(x, y, 10, 7, 13, 11) and key < 1) {
			cout << "Need a key to enter the dinning hall(hint: check forest)" << endl;
			y = 12;
			x = 5;
		}
		if (Pathway(x, y, 19, 15, 14, 11) and key < 2) {
			cout << "Need 2 keys to enter the cave" << endl;
			y = 12;
			x = 14;
		}

		if (Pathway(x, y, 25, 21, 8, 6) and key < 1) {
			cout << "Need 3 keys to enter the final boss" << endl;
			y = 7;
			x = 20;
		}
...
//puzzle2
		...
		if (x == 8 and y == 12 and dinning == true) {
		...
		//puzzle3
		if (x == 9 and y == 12 and dinning2 == true) {
		...
		//puzzle4
		if (x == 17 and y == 12 and cave == true) {
			if (x == 17 and y == 12  and key == 2) {
				scene_four();
				while (puzzle4() == false) {
				}
				...
				cout << "2 keys are needed go back" << endl;
				y++;
				cout << "You found a key!" << endl;
			}
//puzzle5
		}
		if (x == 24 and y == 7 and boss == true) {
			if (x == 24 and y == 7  and key == 3) {
				scene_five();
				while (puzzle5() == false) {
				}
				epilogue();
				...
				cout << "3 keys are needed go back" << endl;
...
//	puzzle3(); //puzzle 3 rewards player with a key to door in scene 4
//	scene_four();
}
...
//	puzzle4(); //rewards the user with another key I guess
//	scene_five();
}
